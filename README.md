...
## 📖 배운점
* Spring 학습 후 첫 프로젝트이기에 화려한 기능 구현보다는 배운 내용을 중점적으로 확인하는 과정을 거쳤음
* Firebase를 사용할 때는 몰랐으나 서버에서 객체를 호출할 때 NullPointerException과 같은 에러처리를 하기가 꽤 까다롭다고 느낌
* Response와 Request를 적절하게 사용하여 REST API를 활용하는 방법을 배움
* 안드로이드 앱과는 다른 웹 앱만의 배치와 방식에 대해 배움
* 전체적인 Spring의 동작 원리와 MVC 패턴의 구조, Controller-Service-Repository 각각의 클래스의 역할에 대해 알게 됨
